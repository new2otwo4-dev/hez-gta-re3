...
cAudioManager::ProcessVehicleHorn(cVehicleParams &params)
{
	...
	if(m_sQueueSample.m_nVolume != 0) {
		// === Õ¿ÿ —¬ﬂŸ≈ÕÕ€… √–¿¿À‹: «¿Ÿ»“¿ Ã¿——»¬¿ Œ“ DLC-»Õƒ≈ —Œ¬ ===
		int safeAudioIndex = params.m_nIndex;
		if(safeAudioIndex >= 4000 || safeAudioIndex > 60) safeAudioIndex = 0; // œÂÂÍÎ˛˜‡ÂÏ ‡‰‰ÓÌ˚ Ì‡ Á‚ÛÍ À‡Ì‰ÒÚ‡ÎÍÂ‡
		// ============================================================

		m_sQueueSample.m_nCounter = 4;
		m_sQueueSample.m_nSampleIndex = aVehicleSettings[safeAudioIndex].m_nHornSample;
		m_sQueueSample.m_nBankIndex = SFX_BANK_0;
		m_sQueueSample.m_bIs2D = false;
		m_sQueueSample.m_nReleasingVolumeModificator = 2;
		m_sQueueSample.m_nFrequency = aVehicleSettings[safeAudioIndex].m_nHornFrequency;
		m_sQueueSample.m_nLoopCount = 0;
		m_sQueueSample.m_nEmittingVolume = 80;
		m_sQueueSample.m_nLoopStart = SampleManager.GetSampleLoopStartOffset(aVehicleSettings[safeAudioIndex].m_nHornSample);
		m_sQueueSample.m_nLoopEnd = SampleManager.GetSampleLoopEndOffset(aVehicleSettings[safeAudioIndex].m_nHornSample);
